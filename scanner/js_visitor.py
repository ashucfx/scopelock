...
class JavaScriptASTVisitor:
    ...
    def _handle_member_expression(
        ...
        if expr_text.startswith("process.env") and not expr_text == "process.env":
            loc = SourceLocation(
                file=file_name,
                line=node.start_point[0] + 1,
                col=node.start_point[1],
            ...
            if node.parent and node.parent.type == "member_expression" and node.parent.start_byte == node.start_byte:
                return
